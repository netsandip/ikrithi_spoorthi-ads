...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$id = $this->db->insert_id();
                ...
                // Assign Role
                if (array_key_exists('role', $additional_data)){
                        if( empty($additional_data['role']))
                        {
                            $strSQL = sprintf("DELETE FROM `acl_user_roles` WHERE `user_id` = %u AND `role_id` = %u",$id,$additional_data['role']);
                        } else {
                            // Delete existing role
                            $strSQL = sprintf("DELETE FROM `acl_user_roles` WHERE `user_id` = %u",$id);
                            $this->db->query($strSQL);
                            $strSQL = sprintf("REPLACE INTO `acl_user_roles` SET `user_id` = %u, `role_id` = %u, `add_date` = '%s'",$id,$additional_data['role'],date ("Y-m-d H:i:s"));
                        }
                        $this->db->query($strSQL);
                        // unset role so it doesn't effect database entry
                        unset($additional_data['role']);
		}
		...
		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column])
		{
			...
			return FALSE;
                ...
                if (array_key_exists('role', $data)){
                        if( empty($data['role']))
                        {
                            $strSQL = sprintf("DELETE FROM `acl_user_roles` WHERE `user_id` = %u AND `role_id` = %u",$user->id,$data['role']);
                        } else {
                            // Delete existing role
                            $strSQL = sprintf("DELETE FROM `acl_user_roles` WHERE `user_id` = %u",$user->id);
                            $this->db->query($strSQL);
                            $strSQL = sprintf("REPLACE INTO `acl_user_roles` SET `user_id` = %u, `role_id` = %u, `add_date` = '%s'",$user->id,$data['role'],date ("Y-m-d H:i:s"));
                        }
                        $this->db->query($strSQL);
                        // unset role so it doesn't effect database entry
                        unset($data['role']);
